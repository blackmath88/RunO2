...
# Clean-air run planner. Additive: nothing above depends on it, and the
# import is guarded so a missing air layer cannot take the main API down.
try:
    from .air.api import router as air_router

    app.include_router(air_router)
except Exception as exc:  # pragma: no cover - defensive
    import logging

    logging.getLogger(__name__).warning("run planner not mounted: %s", exc)
